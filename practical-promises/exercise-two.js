...
function problemA () {
  ...
  // promise version
  // ???
  var promise1 = promisifiedReadFile('poem-two/stanza-01.txt').then(function(stanza){
    blue(stanza);
  });

  var promise2 = promisifiedReadFile('poem-two/stanza-02.txt').then(function(stanza) {
      blue(stanza);
  });
...
function problemB () {
  ...
  .map(filenames, function(filename) {
    return promisifiedReadFile(filename).then(function(stanza) {
      blue(stanza);
    });
  }).then(function(){
    console.log('done');
...
function problemE () {
  ...
  function promisifiedWriteFile (filename, str) {
    ...
    return new Promise(function(resolve, reject) {
        fs.writeFile(filename, str, function(err, file, str) {
          if(err) reject(err);
          else resolve('FOOBAR');
        })
    })
